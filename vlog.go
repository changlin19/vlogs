...
import (
	...
	"vlogs/internal/gopkg.in/natefinch/lumberjack.v2"
)
...
//rotationTime 间隔多久切割一次,最小单位为小时,从服务启动的那个小时的00分00秒开始计算
//timeFormat:用于切分后文件的重命名，默认：2006-01-02T15-04-05.000
//timeFormat格式：时间必须用对应的数字代替。比如:2016--01--02--15--04
// 				  年--2006
// 				  月--01
// 				  日--02
// 				  时--15
// 			      分--04
// 				  秒--05
func SetOutputWithFileWithTimeFormat(serverName, logFilePath, logLevel string, rotationTime int, timeFormat string) {
	newLogger(serverName, logFilePath, logLevel, "file", rotationTime, timeFormat)
}
...
//rotationTime 间隔多久切割一次,最小单位为小时,从服务启动的那个小时的00分00秒开始计算
//timeformat:用于切分后文件的重命名，默认：2006-01-02T15-04-05.000
func newLogger(serverName, logFilePath, logLevel, logOutput string, rotationTime int, timeFormat string) {
	...
	if logOutput == "cmd" {
		...
		if timeFormat != "" {
			lumberjack.SetTimeFormat(timeFormat)
		}
